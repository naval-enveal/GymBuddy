...
/// **the app never holds the Claude API key.** All Claude calls go through the
/// backend, which is the single key-holder (server-side `claude.client` seam +
/// `ANTHROPIC_API_KEY` env). The app only ships the *public* RevenueCat SDK key,
/// which is not a secret.
///
/// This test scans the app's shipping sources for the machinery that would only
/// be present if someone wired the Claude key — or the Anthropic SDK — into the
/// client. It does NOT forbid prose: comments may freely explain *why* the key
/// stays server-side. A regression (a key define, a hardcoded key, an Anthropic
...
void main() {
  ...
  // only show up if the app started holding the key or talking to Anthropic
  ...
  final forbidden = <RegExp>[
    ...
    // A live Anthropic API key prefix accidentally hardcoded/committed.
    RegExp('sk-ant-'),
    // A dependency on the Anthropic SDK (npm `@anthropic-ai/...`, the Dart
    // `anthropic` package, etc.) — AI must be reached only via the backend.
    RegExp(r'anthropic[-_/]ai', caseSensitive: false),
    RegExp('package:anthropic'),
  ];
  ...
  test('app sources never reference the Claude API key or the Anthropic SDK',
      () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'The app must never hold the Claude API key or depend on the '
          'Anthropic SDK — AI is reached only through the gated backend '
